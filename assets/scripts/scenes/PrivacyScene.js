class PrivacyScene extends Phaser.Scene {
	...
	createTextBlock() {
		let content = [
			...
			'1. INTRODUCTION',
			'THIS IS PRIVACY POLICY ${this.config.nameGame}, WHICH EXPLAINS HOW IS COLLECT, USE AND SHARE DATA THROUGH MOBILE APPLICATIONS, OUR PAGES OR PRESENCE ON THIRD PARTY WEBSITES',
			'AND OTHER PLATFORMS, WEBSITES PRODUCTS, OFFERS AND SERVICES THAT WE OFFER FROM TIME TO TIME, ALL OF WHICH TOGETHER WE REFER AS SERVICES OR SERVICES',
			...
			`PERSONAL DATA MEANS ANY INFORMATION FROM WHICH THE PLAYER CAN BE DIRECTLY	OR INDIRECTLY IDENTIFIED SUCH AS A NAME, LOCATION, EMAIL ADDRESS,	COMMUNICATION LANGUAGE,'
			'IP ADDRESS, IDFV (ID FOR VENDOR) OR ANDROID ID,	ADVERTISEMENT ID, PUSH NOTIFICATIONS TOKEN, GAME HISTORY, NETWORK LOGS,	COOKIE ONLINE IDENTIFIER, GENDER, FINANCIAL DATA,'
			'ETC., (“PERSONAL DATA”). YOUR PERSONAL DATA ARE PROCESSED ONLY TO THE EXTENT STRICTLY NECESSARY,	DEPENDING ON THE PARTICULAR PURPOSE OF THE PROCESSING, AS DEFINED IN POINT 6.`,
		]
		let txtCont = [
			'1. QQQQQQQQQQQQQQQQQQQ QQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQ QQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQ QQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQQ ${this.config.nameGame}',
